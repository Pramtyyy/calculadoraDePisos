...
def inicializar_banco():
    with sqlite3.connect(DATABASE) as banco:
        ...
        colunas = [
            linha[1]
            for linha in banco.execute("PRAGMA table_info(app_data)")
        ]

        if "vendas" not in colunas:
            banco.execute(
                "ALTER TABLE app_data ADD COLUMN vendas TEXT NOT NULL DEFAULT '[]'"
            )

        banco.execute(
            """
            INSERT OR IGNORE INTO app_data (id, pisos, vendas)
            VALUES (1, '[]', '[]')
            """
        )


def salvar_foto(dados_base64):
    if "," in dados_base64:
        dados_base64 = dados_base64.split(",", 1)[1]

    bytes_imagem = base64.b64decode(dados_base64)
...
def ler_pisos():
    with sqlite3.connect(DATABASE) as banco:
        registro = banco.execute(
            "SELECT pisos FROM app_data WHERE id = 1"
        ).fetchone()
...
def ler_vendas():
    with sqlite3.connect(DATABASE) as banco:
        registro = banco.execute(
            "SELECT vendas FROM app_data WHERE id = 1"
        ).fetchone()
        ...
        if self.path == "/api/foto":
            try:
                ...
                if len(corpo) < tamanho:
                    raise ValueError(
                        "Corpo incompleto: conexao interrompida"
                    )
                ...
                if "foto" not in dados:
                    raise ValueError(
                        "Campo 'foto' nao encontrado"
                    )

                nome_arquivo = salvar_foto(
                    dados["foto"]
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        if (
            getattr(erro, "winerror", None) == 10048
            or erro.errno == 98
        ):
            print(
                "A porta 8000 ja esta em uso. "
                "Feche o outro servidor ou use "
                "a instancia que ja esta rodando."
            )
        else:
            print(
                f"Nao foi possivel iniciar o servidor: {erro}"
            )
    ...
    print(
        "Use no celular o endereco "
        "http://IP_DESTE_COMPUTADOR:8000"
    )
